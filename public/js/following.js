...
define(['iAlert', 'Util', 'User'], function(iAlert, Util, User) {
	...
	function findFollowingsByPage(followingId, config, callback) {
		$.ajax({
			url: '/api/users/' + followingId + '/followings',
			type: 'GET',
			data: config,
			dataType: 'json',
			timeout: 15000,
	...
	var followingId = GNS.u._id;

	findFollowingsByPage(followingId, config, function(data) {
